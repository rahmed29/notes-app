...
function showTodo(hereForInsertion) {
  ...
  const bookDiffContent = createPopupWindow({
    noAnimation: false,
    cleanups: [deleteCalendar, clearTaskTippys],
  });
  ...
  seePast.addEventListener("click", () => {
    seePast.hasAttribute("data-enabled")
      ? renderTaskList(false, taskList)
...
async function initializeTodo() {
  ...
  if (response.ok) {
    ...
    events = json.data.current;
    pastEvents = json.data.past;
...
async function saveTodo() {
  const response = await fetch("/api/save/notebooks/todo__list", {
    ...
    body: JSON.stringify({
      content: [JSON.stringify(events), JSON.stringify(pastEvents)],
...
function renderTaskList(lookingAtPast, taskList, constraint) {
  ...
  arr.forEach((task) => {
          ...
          click: function () {
            ...
            this.addEventListener(
              ...
              function () {
                ...
                attemptRemoval([calendar.getEventById(task.id)]);
    ...
    label.addEventListener("blur", () => {
      if (
        label.innerText.replaceAll("\n", "") &&
        label.innerText.replaceAll("\n", "") !== task.title
      ) {
        task.title = label.innerText.replaceAll("\n", "");
        saveTodo();
      } else {
        label.innerText = task.title;
      }
